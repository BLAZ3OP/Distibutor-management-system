<?php
  ...
  function isCustomer($name, $contact_number) {  // add new invoice.js on line 152
  ...
  function isInvoiceExist($invoice_number) {  // add new invoice.js on line 164 , add new purchase on line 31
    require "db_connection.php";
    if($con) {
  ...
  function isMedicine($name) {  // add new invoice.js on line 176 
    require "db_connection.php";
    if($con) {
  ...
  function createMedicineInfoRow() {  // add new purchase.php on line 95
    $row_id = $_GET['row_id'];
    ...
    <div class="row col col-md-12">
      ...
      <div class="col col-md-2">
        <button class="btn btn-primary" onclick="addRow();">
          <!-- add new invoice.js on line 15 -->
          <!-- add new purchase.js on line 25 -->
          <i class="fa fa-plus"></i>
        </button>
        <button class="btn btn-danger"  onclick="removeRow('<?php echo $row_id ?>');">
          <!-- add new invoice.js on line 37 -->
          <!-- add new purchase.js on line 46 -->
  ...
  function getInvoiceNumber() {  // add new invoice.js on line 46
    require 'db_connection.php';
    if($con) {
      $query = "SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'gland_pharma' AND TABLE_NAME = 'invoices';"; // to get auto increment num in add sales invoice page
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
  ...
  function fill($name, $column) {  // add new invoice.js on line 84
    require 'db_connection.php';
    if($con) {
      $query = "SELECT * FROM medicines_stock WHERE UPPER(NAME) = '$name'";
      $result = mysqli_query($con, $query);
  ...
  function checkAvailableQuantity($name) {  // add new invoice.js on line 124
    require "db_connection.php";
    if($con) {
      $query = "SELECT QUANTITY FROM medicines_stock WHERE UPPER(NAME) = '$name'"; //calculate the quantity
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
  ...
  function updateStock($name, $batch_id, $quantity) {  // add new invoice.js on line 308
    require "db_connection.php";
    if($con) {
    ...
    }  // used to increase the quantity by name
  }
  ...
  function addSale() {  // add new invoice.js on line 319
  ...
  function addNewInvoice() {  // add new invoice.js on line 330
    $customer_id = getCustomerId(strtoupper($_GET['customers_name']), $_GET['customers_contact_number']);
