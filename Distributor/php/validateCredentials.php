<?php
  ...
  function isSetupDone() {  // index.js - 80
  ...
  function verifyEmailNumber() {  // index.js - 103
    require "db_connection.php";
    if($con) {
      $email = $_GET["email"];
      $contact_number = $_GET["contact_number"];
  ...
  function updateUsernamePassword() {  // index.js - 122
    require "db_connection.php";
    if($con) {
      $username = $_GET["username"];
      $password = $_GET["password"];
  ...
  function validatePassword() {  // my profie.js - 68
    require "db_connection.php";
    if($con) {
      $password = $_GET["password"];
  ...
  function updateAdminInfo() {
    ...
    if($con) {
      ...
      $query = "UPDATE admin_credentials SET DISTY_NAME = '$pharmacy_name', ADDRESS = '$address', EMAIL = '$email', CONTACT_NUMBER = '$contact_number', USERNAME = '$username'";
      $result = mysqli_query($con, $query);
  ...
  if(isset($_GET['action']) && $_GET['action'] == 'change_password')
    changePassword(); // my profie.js - 68
